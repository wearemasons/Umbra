...
function drag(
  ...
  updateNodePosition: (nodeId: Id<'knowledgeGraphNodes'>, x: number, y: number) => void
) {
  function dragstarted(event: d3.D3DragEvent<SVGCircleElement, ResearchPaperNode, ResearchPaperNode>) {
  ...
  function dragged(event: d3.D3DragEvent<SVGCircleElement, ResearchPaperNode, ResearchPaperNode>) {
  ...
  function dragended(event: d3.D3DragEvent<SVGCircleElement, ResearchPaperNode, ResearchPaperNode>) {
  ...
  return d3.drag<SVGCircleElement, ResearchPaperNode>()
...
const Tooltip: React.FC<{ 
  ...
  return (
    ...
    >
      ...
      <div className="text-sm text-gray-300 mb-1">Authors: {paper.authors.join(', ')}</div>
      <div className="text-sm text-cyan-300 mb-1">Year: {paper.publicationYear} | Citations: {paper.citationCount}</div>
...
const ResearchPaperGraph: React.FC<ResearchPaperGraphProps> = ({ 
  ...
  const simulationRef = useRef<d3.Simulation<ResearchPaperNode, undefined> | null>(null);
  const d3ContainerRef = useRef<d3.Selection<SVGSVGElement, unknown, null, undefined> | null>(null);

  // State for graph dimensions and UI elements
  const [width, setWidth] = useState(800);
  const [height, setHeight] = useState(600);
  const [tooltip, setTooltip] = useState<{ paper: ResearchPaperNode | null, position: { x: number, y: number } }>({ 
    paper: null, 
    position: { x: 0, y: 0 } 
  });
  const [fixedPositions, setFixedPositions] = useState<Record<string, { x: number, y: number }>>({});
  ...
  const updateNodePosition = useCallback((nodeId: Id<'knowledgeGraphNodes'>, x: number, y: number) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation<ResearchPaperNode>()
      // Link force: keeps connected nodes at a distance based on relationship strength
      .force('link', 
        d3.forceLink<ResearchPaperNode, GraphEdge>()
          .id(d => d._id)
  ...
  useEffect(() => {
    if (!simulationRef.current || !d3ContainerRef.current || nodes.length === 0) return;
    ...
    nodes.forEach(node => {
      if (fixedPositions[node._id]) {
        ...
        node.x = width / 2 + (Math.random() - 0.5) * Math.min(width, height) * 0.5;
        node.y = height / 2 + (Math.random() - 0.5) * Math.min(width, height) * 0.5;
        node.fx = null;
        node.fy = null;
      }
    });
    ...
    (simulation.force('link') as d3.ForceLink<ResearchPaperNode, GraphEdge>).links(extendedEdges);
    ...
    const linkEnter = links.enter()
      ...
      .attr('stroke', d => {
        ...
        switch(d.relationshipType) {
          case 'cites': return '#7D3C98'; // Purple for citation relationships
          case 'references': return '#2874A6'; // Dark blue for reference relationships  
          case 'similar': return '#D68910'; // Amber for similarity relationships
          case 'follows': return '#1D8348'; // Green for sequential relationships
          case 'builds-on': return '#CB4335'; // Red for building-on relationships
          default: return '#5D6D7E'; // Gray for default
        }
      })
      .attr('stroke-width', d => Math.max(1, d.strength * 3)) // Thicker for stronger relationships
      .attr('stroke-opacity', 0.6)
      .attr('marker-end', d => d.relationshipType === 'cites' || d.relationshipType === 'references' 
    ...
    const curvePath = (source: ResearchPaperNode, target: ResearchPaperNode) => {
    ...
    nodeEnter.append('text')
      ...
      .text(d => d.title.length > 20 ? d.title.substring(0, 20) + '...' : d.title) // Truncate long titles
      .attr('font-size', '12px')
  ...
  return (
    <div className="w-full h-full relative bg-gradient-to-br from-gray-900 to-black rounded-lg overflow-hidden">
      ...
      <div className="absolute top-2 left-2 text-white text-xs z-10 bg-black bg-opacity-50 p-1 rounded">
        Nodes: {nodes.length}, Edges: {edges.length}, Width: {width}, Height: {height}
      </div>
